...
function removeLastOccurrences(str, value) {
  ...
  const value1 = value.split('').reverse().join('');
  return str1.replace(value1, '').split('').reverse().join('');
}
...
function countVowels(str) {
  if (str > 0) {
    return str.match(/[aeiouyAEIOUY]/).length;
  }
  return 0;
}
...
//     if (vowels.includes(str.toLowerCase[i])) countV += 1;
//   }
//   return countV;}
//   let counter = 0;
...
function isPalindrome(str) {
  const str1 = str.toLowerCase.replace(' ', '').match(/[^,!?]/gi);
...
function findLongestWord(sentence) {
  ...
  return sentence1.sort((a, b) => b.length - a.length)[0];
}
